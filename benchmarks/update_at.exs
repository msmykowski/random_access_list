...
Benchee.run(
  %{
    "standard" => fn {standard, _ral, index}-> List.update_at(standard, index, &(&1 + 1)) end,
    "random_access_list" => fn {_standard, ral, index} -> RandomAccessList.update_at(ral, index, &(&1 + 1)) end,
  },
  inputs: %{
    "ten" => {ten, ten_ral, 5},
